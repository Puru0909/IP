...
#             cv2.circle(frame, (x, y), 2, (0, 255, 0), 3)  # Draw center
#             cv2.putText(frame, f"Radius: {r}", (x - 50, y - r - 10), 
...
#     cv2.putText(frame, f"Contours: {len(contours)}", (50, 50), 
...
while True:
    ...
    # Detect circles using Hough Transform (for basketball and net ring)
    circles = cv2.HoughCircles(
    ...
    if circles is not None:
        ...
        for (x, y, r) in detected_circles[0, :]:
            ...
            cv2.putText(frame, f"Ball/Ring: ({x}, {y}) R: {r}", (x - 50, y - r - 10), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
    ...
    # Apply contour detection
    ret, binary = cv2.threshold(morphed, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    contours, _ = cv2.findContours(binary, mode=cv2.RETR_TREE, method=cv2.CHAIN_APPROX_SIMPLE)
